...
os.mkdir('./cgan_result') if not os.path.exists('./CGAN/cgan_result') else 0
...
for epoch in range(epoch_num):
    for i, (images, label) in enumerate(train_loader):
        ...
        onehot_label = torch.zeros(image_num, label_num).scatter_(1, label.view(-1, 1), 1).to(device)
    ...
    if (epoch+1) == 1:
        images = images.reshape(image_num, 1, 28, 28)
        save_image(denorm(images), './cgan_result/real_images.png')
    if (epoch+1) % 10 == 0:
        fake_images = fake_images.reshape(image_num, 1, 28, 28)
